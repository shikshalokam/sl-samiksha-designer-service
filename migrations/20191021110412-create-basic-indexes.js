module.exports = {
  async up(db) {
    ...
    await db.collection('draftQuestions').createIndex( { criteriaId: 1 }, { background : 1} )
    await db.collection('draftQuestions').createIndex( { userId: 1 }, { background : 1} )
    await db.collection('draftQuestions').createIndex( { frameworkId: 1 }, { background : 1} )
    await db.collection('draftQuestions').createIndex( { evidenceMethod: 1 }, { background : 1} )
